...
**Goal:** 交付一个建在 OpenClaw（小龙虾）+ 飞书 CLI 上的跨境 B2C 物流客诉自动处理 Agent，含可量化评测记分卡，作为安全 AI Agent 模式的工程落地基线与方法论文档。

**Architecture:** 核心业务逻辑写成可导入的 Python 库（`src/`），TDD 覆盖；OpenClaw Skill（`skill/SKILL.md` + `scripts/` + `references/`）在线编排并调 `src/` 与 `lark-cli`；离线评测（`eval/`）直接 import `src/pipeline.py` 用 DeepSeek v4-pro 跑标注集，与线上国产模型路径共用 `skill/references/` 里的提示词/规则（单一事实源，DRY）。

**Tech Stack:** Python 3.11 · pytest · OpenAI SDK（离线评测/开发用 DeepSeek v4-pro）· OpenClaw（腾讯云 Lighthouse）· `@larksuite/cli` · 飞书自建应用 · 线上 OpenClaw 绑 DeepSeek v4-flash
...
- **在线 vs 离线两条路径，共用规则源**：`skill/references/` 下的 `classify_prompt.md`、`draft_prompt.md`、`escalation_rules.md`、`policy_kb.json` 是唯一事实源。`SKILL.md`（在线，OpenClaw 国产模型读它）与 `src/`（离线，DeepSeek v4-pro）都加载这些文件，禁止各写一份。
- **LLM 在接口后面**：`src/llm.py` 定义 `LLMClient` 协议。单测用 `FakeLLM`（确定性，不联网）。离线评测/开发用 `DeepSeekClient`。在线由 OpenClaw 绑定的 DeepSeek v4-flash 按 `SKILL.md` 执行——不在本仓代码内调用。
- **机密不进 git**：`.env` 存 `DEEPSEEK_API_KEY`、飞书 `APP_ID/APP_SECRET`、国产模型 key，**gitignore**。合成数据（政策/订单/测试集）是虚构的，正常提交。
- **P0 含人工步骤**：开账号、买服务器、OAuth 点授权、填密钥由**用户本人**完成（AI 助手给出逐步指引并在拿到凭据后跑验证命令）。AI 助手不创建账号、不代付。
- **提交节奏**：每个 Task 末尾提交一次。提交信息用 `feat:/test:/docs:/chore:` 前缀。
...
├── src/
│   ├── __init__.py
│   ├── llm.py                     # LLMClient 协议 + FakeLLM + DeepSeekClient
...
OpenClaw 控制台 → 模型 → 选 DeepSeek，填该模型 API Key（用户的 key，写入 `.env` 备份键 `ONLINE_LLM_KEY=...；ONLINE_LLM_PROVIDER=deepseek；ONLINE_LLM_MODEL=deepseek-v4-flash`，控制台内填）。
...
FEISHU_APP_SECRET=
ONLINE_LLM_KEY=
ONLINE_LLM_PROVIDER=deepseek
ONLINE_LLM_MODEL=deepseek-v4-flash
...
- [ ] **Step 3: 本地联调验证（用 DeepSeek v4-pro）**

Run: `DEEPSEEK_API_KEY=$KEY python skill/scripts/run_pipeline.py "BF1001 发了好久还没到"`
...
Run: `DEEPSEEK_API_KEY=$KEY python eval/run_eval.py --which agent --limit 3`
...
Run: `DEEPSEEK_API_KEY=$KEY python eval/run_eval.py --which agent  > eval/score_agent.json`
Run: `DEEPSEEK_API_KEY=$KEY python eval/run_eval.py --which baseline > eval/score_baseline.json`
Expected: 两份记分卡 JSON。
...
包含小节：项目一句话定位 / 架构图（复制 spec §4 的 ASCII）/ **头条指标表**（从 `eval/SCORECARD.md` 填真实数字，禁占位）/ 技术栈（OpenClaw+飞书CLI+腾讯云+DeepSeek；离线评测 deepseek-v4-pro）/ 本地复现步骤（venv + pytest + `eval/run_eval.py`）/ 线上部署步骤（P0 实测：腾讯云模板、飞书应用权限、lark-cli 授权、命令面）/ 演示录屏链接占位（P5.2 后填）/ 免责声明（数据全合成）。
...
- [ ] **Step 1: 录制脚本化演示（用户操作飞书，AI 助手给逐步台本）**
...
```bash
git add CASE.md
git commit -m "docs: 一页纸项目摘要（真实数字）"
...
- §11 LLM 决策（线上国产/离线 DeepSeek v4-pro）→ 约定 + P0.3 + P2.1（DeepSeekClient 仅离线）+ run_eval 用 DeepSeek v4-pro。
- 无未覆盖需求。
