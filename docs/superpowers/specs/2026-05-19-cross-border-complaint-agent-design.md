...
跨境 B2C 物流是一个对**安全可核查 AI 助手**有真实需求的场景：商户在飞书/邮件/IM 多渠道发来中英客诉与咨询，处理时常涉及时效、丢件赔付、清关、对账等敏感判断——单一 prompt 的 LLM 容易乱承诺时效或赔付金额、对运营造成真实风险。本项目以这个场景为靶，做一次端到端的**安全 AI Agent 工程探索**：建在 2026 年活跃的 Agent 工具栈（OpenClaw + 飞书 CLI + DeepSeek）之上，验证三件事——

1. **可追溯的政策 grounding** 能否在工程上稳定实现（每条事实句必引政策条款 ID 或订单事实）。
2. **零过度承诺护栏**（带"已引用即视为有据"豁免）能否在对抗子集上保持 0% 触发率。
3. **规则化决策 + LLM 共决**能否在小规模标注集上接近或反超裸 LLM。

目标产出：一份可复现、有 SCORECARD 数字、含真实生产部署（腾讯云 + 飞书）的工程基线，沉淀方法论文档与每一轮缺陷修正记录，便于后续在更广分布上验证或迁移到相邻业务领域。
...
产出：一页记分卡（终端表 + README 一张表）。这是 SCORECARD 头条数字的唯一来源。
...
6. 一页纸项目摘要（≤100 字 + 截图/链接，可用作 README/简介复制粘贴）
