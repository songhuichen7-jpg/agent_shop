...
- 环境变量：`/home/node/agent_shop/.env`（`DEEPSEEK_*`、`FEISHU_BITABLE_APP_TOKEN`、`FEISHU_BITABLE_TABLE_ID`、`FEISHU_HUMAN_CHAT_ID`，可选 `FEISHU_TASK_ASSIGNEE_ID`）。用前 `set -a; . /home/node/agent_shop/.env; set +a`。
